...
setwd("~/Documents/Coursera/DevelopingDataProducts_09/FirstPrez/test")
library(dplyr)
...
table_results <- tbl_df(training.indep)  %>% group_by(Team) %>% summarize(Played=n())
table_results <- merge(table_results,tbl_df(training.indep) %>% group_by(Team) %>% summarize(Won=sum(Result=="W")))
table_results <- merge(table_results,tbl_df(training.indep) %>% group_by(Team) %>% summarize(Drawn=sum(Result=="D")))
table_results <- merge(table_results,tbl_df(training.indep) %>% group_by(Team) %>% summarize(Lost=sum(Result=="L")))
table_results <- merge(table_results,tbl_df(training.indep) %>% group_by(Team) %>% summarize(For=sum(Goals)))
table_results <- merge(table_results, tbl_df(training.indep) %>% group_by(Opponent) %>% summarize(Against=sum(Goals)), by.x="Team", by.y="Opponent")
table_results <- table_results %>% mutate(Diff=For-Against, Points=Won*3+Drawn) %>% arrange(desc(Points))

knitr::kable(table_results, caption = "League Table Results")
```

--- .class #id 

### Average Scored and Conceded Goals for each Team 
```{r, echo=FALSE, results="asis", message=FALSE, fig.width=12, fig.height=4 }
library(dplyr)
...
### Home Advantage
```{r echo=FALSE, results="asis", message=FALSE, fig.width=13, fig.height=1.2}
library(reshape2)
...
```{r echo=FALSE, results="asis", message=FALSE, fig.width=12, fig.height=2}
...
```{r echo=FALSE, results="asis", message=FALSE, fig.width=12.3, fig.height=2}
...
knitr::kable(fixtures, caption = "Fixture")
```
...
DCoptimRhoFn <- function(par) {
  rho <- par[1]
  DCLogLik(training$HomeGoals, training$AwayGoals, home.expected, away.expected, rho)
}
res <- optim(par=c(0.01), fn=DCoptimRhoFn, control=list(fnscale=-1), method='CG')
```

```{r echo=FALSE, results="hide", fig.width=12, fig.height=4}
maxgoal <- 12
...
g <- ggplot(data=mplt, aes(x=factor(match,levels=rev(unique(match))), y= value, fill=`Home Match Result`))
g <- g + geom_bar(stat = "identity")+coord_flip()
g <- g + theme_bw(base_family = "Avenir", base_size = 15)
g <- g + xlab("") + ylab("Probabilities") + ggtitle("Match Prediction")
g
```
